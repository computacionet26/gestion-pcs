...
export default function ({setSession}){
    ...
    async function handleSubmit(event) {
        ...
        try {
            const hola = await axios({
                method: "POST",
                url:  `${import.meta.env.VITE_API_URL}/device/report/${id.current.value}`,
                data: {
    ...
    return (
        <div className='w-5/6 md:w-auto'>
            ...
            <form action={`${import.meta.env.VITE_API_URL}/user/login`} onSubmit={handleSubmit} method="post" className='flex flex-col gap-6 bg-white p-8 rounded-b shadow-xl'>
