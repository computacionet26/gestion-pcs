...
function Login({setSession}){
    ...
    async function handleSubmit(event) {
        ...
        try {
            const res = await post('http://10.120.3.179:3000/user/login', {
                username: email,
                password
            })
    ...
    return (
        <div className='w-11/12 sm:w-auto'>
            <form action="http://10.120.2.179:3000/user/login" onSubmit={handleSubmit} method="post" className='flex flex-col gap-6 bg-white p-8 rounded shadow-xl'>
